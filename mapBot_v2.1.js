...
class MCBot {
  ...
  async start(n){//建造地圖畫
    ...
    for(let i=0;i<this.mapData.blocks.length;i++){
      ...
      //當還有剩餘方塊時
      while(this.mapData.blocks[i].pos.length>0){
        logMessage("同種類方塊剩餘："+this.mapData.blocks[i].pos.length);
        //取得材料
        await this.equipItem(name);
        while(!this.bot.heldItem||this.bot.heldItem.name!==name||this.bot.heldItem.count==0){
          await this.materialGet(name,this.mapData.blocks[i].pos.length,shulkerBoxes);
          await this.equipItem(name);
        ...
        for(let d=6;d<Math.max(this.mapData.size[0],this.mapData.size[1],this.mapData.size[2]);d+=2){
        ...
        for(let count=0;count<3;count++){
          if(await this.pathTo(targ.x,targ.y+1,targ.z))break;
        }
        await wait(50);
        //if(this.forcedMoveFlag===true)continue;
        botPos=this.bot.entity.position.floored();
        if(botPos.x===targ.x&&botPos.y===targ.y+2&&botPos.z===targ.z){
          ...
          for(let j=0;j<this.mapData.blocks[i].pos.length;j++){
            ...
            if(rx<=4&&rx>=-4&&rz<=4&&rz>=-4&&ry<=5&&ry>=-3){
              let block=this.bot.blockAt(new Vec3(x,y,z));
              if(block&&block.name!=="air")removeList.push(j);
              else if(rx**2+(ry-1)**2+rz**2<=30)placeList.push([x,y,z,face]);
            };
          }
          logMessage("需要建造："+placeList);
          for(let k=removeList.length-1;k>=0;k--)this.mapData.blocks[i].pos.splice(k,1);//可能改成建造狀態=1，然後另建一個索引
  ...
  async pathTo(x,y,z,d){
    if(this.bot.entity.position.equals(new Vec3(x,y,z)))return true;
    ...
    for(let j of path){
      ...
      if(this.forcedMoveFlag===true)return false;
    }
    await wait(60);
    if(this.forcedMoveFlag===true)return false;
    return true;
